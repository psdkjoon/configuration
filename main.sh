...
sudo pacman -Syu git curl wget hyprland kitty rofi waybar mako swaylock swayidle hyprpaper xdg-desktop-portal-hyprland zsh zoxide fd ripgrep bat gping thefuck tealdeer neovim btop tgpt 7zip jq poppler fzf resvg imagemagick npm wl-clipboard mpv ttf-jetbrains-mono-nerd
...
sudo mv nvim yazi nekoray $USERHOME/.config/

sudo mkdir $USERHOME/.config/kitty

sudo mv kitty.conf $USERHOME/.config/kitty/

sudo mv nekobox $USERHOME/Desktop/
